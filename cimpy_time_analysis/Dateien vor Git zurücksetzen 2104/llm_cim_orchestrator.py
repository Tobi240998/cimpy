...
def _detect_topology_intent(user_input: str, analysis_plan: dict | None = None) -> dict:
    if analysis_plan:
        ...
        if intent == "topology_query":
            resolved_intent = topology_scope if topology_scope in {"neighbors", "component", "path"} else "neighbors"
            return {
                "is_topology": True,
        ...
        if needs_topology_graph and topology_scope in {"neighbors", "component", "path"}:
            return {
                "is_topology": True,
...
def handle_user_query(
    ...
    if "SvPowerFlow" in state_detected:
        if metric is None:
            print("metric_fallback_used:", equipment_type)
