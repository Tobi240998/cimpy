...
# Auflösen der Vergleiche 
def _resolve_comparison_definition(
    user_input: str,
    resolved_object: Any,
    ...
    equipment_name = (
        _safe_name(resolved_object) or getattr(resolved_object, "mRID", None)
        if resolved_object is not None else None
    )

    allowed = {
        name: spec
        for name, spec in _COMPARISON_DEFINITIONS.items()
    ...
    try:
        ...
        definitions_text = "\n".join(
            f"- {name}: "
            f"state_type={spec.get('required_state_type')} | "
            f"sv_metric={spec.get('sv_metric')} | "
            f"base_attributes={spec.get('base_attributes')} | "
            f"mode={spec.get('comparison_mode')}"
        ...
        if not decision.should_execute:
            return {
                "status": "error",
                "error": "comparison_not_safely_resolved",
                "details": "LLM decided that the comparison request is not safely resolvable.",
        ...
        if comparison_type not in allowed:
            return {
                "status": "error",
                "error": "comparison_type_not_allowed",
                "details": f"LLM returned comparison_type={comparison_type!r}, which is not allowed for equipment class '{equipment_class}'.",
                "llm_decision": decision.model_dump() if hasattr(decision, "model_dump") else decision.dict(),
                "allowed_comparison_types": sorted(allowed.keys()),
            }
...
def _resolve_candidate_value_deep(
    ...
    *,
    ...
    """
    Targeted graph/value traversal for a concrete requested attribute.

    Important behavior:
    - Only returns a value if the concrete target_attribute is found.
    - Does NOT fall back to generic readable fields like name, mRID, description.
    - Traverses lists and nested object references, but remains attribute-specific.
    """
    visited: set[int] = set()

    def _not_found(path: List[str], reason: str) -> Dict[str, Any]:
        return {
            "status": "not_found",
            "value": None,
            "resolved_field": None,
            "resolution_path": " -> ".join(path + [reason]) if path else reason,
        }
    ...
    def _walk(value: Any, depth: int, path: List[str]) -> Dict[str, Any]:
        ...
        if depth >= max_depth:
            return _not_found(path, "max_depth_reached")

        # Direct scalar values are only valid if they are already the concrete
        # value of the requested target attribute. That is represented by the
        # last path element being exactly the target attribute.
        if _is_simple_readable_value(value):
            if path and path[-1] == target_attribute:
                return {
                    "status": "ok",
                    "value": value,
                    "resolved_field": target_attribute,
                    "resolution_path": " -> ".join(path),
                }
            return _not_found(path, "direct_scalar_without_target_match")

        obj_id = id(value)
        if obj_id in visited:
            return _not_found(path, "cycle_detected")
        visited.add(obj_id)

        # List handling: recurse into items, but still only accept matches that
        # resolve the concrete target_attribute.
        if isinstance(value, list):
            for idx, item in enumerate(value[:max_list_items]):
                result = _walk(item, depth + 1, path + [f"[{idx}]"])
                if result.get("status") == "ok":
                    return result
            return _not_found(path, "list_exhausted")

        # Object handling:
        # 1) If the object itself has the concrete target attribute, follow only that.
        try:
            if hasattr(value, target_attribute):
                direct_target_value = getattr(value, target_attribute, None)
                if direct_target_value is not None:
                    result = _walk(
                        direct_target_value,
                        depth + 1,
        ...
        # 2) Otherwise recurse into navigable child references, but do not read
        #    arbitrary simple fields from the current object.
        ...
        return _not_found(path, "no_target_attribute_found")
...
def _resolve_base_candidate_agentically(
    ...
    try:
        ...
        if direct_value is not None:
            if _is_simple_readable_value(direct_value):
                return {
                    "status": "ok",
                    ...
                    "value": direct_value,
                    "searched_scopes": searched_scopes,
                    "resolution_path": f"direct_or_canonical_reader -> {candidate_name}",
                    "resolved_field": candidate_name,
                }

            deep_result = _resolve_candidate_value_deep(
                direct_value,
                target_attribute=candidate_name,
            )
    ...
    try:
        if hasattr(resolved_object, candidate_name):
            ...
            if raw_value is not None:
                if _is_simple_readable_value(raw_value):
                    return {
                        "status": "ok",
                        "candidate": candidate_name,
                        "resolved_attribute": candidate_name,
                        "value": raw_value,
                        "searched_scopes": searched_scopes,
                        "resolution_path": f"direct_object_attribute -> {candidate_name}",
                        "resolved_field": candidate_name,
                    }
    ...
    try:
        for child_name, child_value in _iter_navigation_candidates_from_object(resolved_object):
            deep_result = _resolve_candidate_value_deep(child_value, target_attribute=candidate_name)
    ...
    #    Keeping this because it already exists and is useful, but only
    #    after the generic search stages above. (-> Refactoring-Kandidat zum Löschen)
    # --------------------------------------------------------------
...
def _read_cim_base_values_with_services(
    ...
    if effective_requested_attributes:
        ...
        for attr_name in effective_requested_attributes:
            if attr_name in {"lowVoltageLimit", "highVoltageLimit"}:
                resolution = _resolve_base_attribute_with_expanded_search(
                    services=services,
                    resolved_object=resolved_object,
                    attr_name=attr_name,
                )
            ...
            if resolution.get("status") == "ok":
                resolved_attr = resolution.get("resolved_attribute") or resolution.get("attribute") or attr_name
...
def _query_cim_with_services(
    ...
    if classification and classification.get("intent") == "topology_query":
