...
class PFPlanner:
    ...
    def __init__(self, project_name: str = DEFAULT_PROJECT_NAME, debug_mode: bool = True, registry=None):
        ...
        self.planner_prompt = ChatPromptTemplate.from_messages([
            (
                "system",
                "You are the primary classifier for a PowerFactory domain agent.\n"
                "Your task is to classify the user request into exactly one supported intent.\n"
                "Do not build a detailed tool plan. Standard plans already exist for all supported intents.\n\n"
                ...
                "Intent definitions:\n"
                "- load_catalog: The user asks which PowerFactory objects exist in the active project. "
                "This includes loads, transformers, lines, buses, switches, generators, or similar supported object catalogs.\n"

                "- change_load: The user asks to increase, reduce, set, or otherwise modify a load value. "
                "If the same request also asks for voltage, loading, active power, reactive power, or other result metrics after the load change, "
                "keep intent=change_load. These result views are handled inside the standard load-change workflow.\n"

                "- topology_query: The user asks about neighbors, connectivity, connected elements, attached assets, network topology, or what is electrically connected to an object.\n"

                "- change_switch_state: The user asks to open, close, toggle, switch on/off, or change the state of a switch, breaker, disconnector, coupler, or similar switching device.\n"

                "- query_element_data: The user asks for values, parameters, attributes, base data, technical data, setpoints, nominal values, limits, bounds, min/max values, "
                "result values, calculated values, load-flow values, or operational states of a specific PowerFactory object.\n"
                "Examples include Nennspannung, Sollspannung, Spannung, obere/untere Spannungsgrenze, Grenzwerte, Auslastung, Wirkleistung, Blindleistung, Schalterstellung, "
                "technical attributes such as uknom or m:u, and base/result data of buses, lines, loads, transformers, generators, or switches.\n"
                "Plural or compound data requests such as 'obere und untere Spannungsgrenze' are still one query_element_data request.\n"

                "- list_element_attributes: The user asks which attributes, fields, parameters, or data columns are available for an object, without asking for the value of a specific attribute.\n"

                "- unsupported_powerfactory_request: The request is outside the currently supported PowerFactory workflows.\n\n"

                "Decision rules:\n"
                "- Prefer a supported intent when the request clearly fits one of the definitions.\n"
                "- Do not decompose compound attribute requests. Attribute selection can handle multiple attributes.\n"
                "- Do not classify object data requests as topology_query unless the user asks about connectivity or neighbors.\n"
                "- Do not classify catalog/listing requests as query_element_data when the user only asks which objects exist.\n"
                "- Keep required_steps empty or minimal. Do not invent step names.\n"
                "- safe_to_execute should be true for clearly supported read-only queries and clearly specified supported mutations.\n"
                "- Use missing_context only when information required for the standard workflow is absent.\n\n"

                "Examples:\n"
                "- 'Welche Lasten gibt es in PowerFactory?' -> load_catalog\n"
                "- 'Welche Busse gibt es in PowerFactory?' -> load_catalog\n"
                "- 'Reduziere Last A um 2 MW' -> change_load\n"
                "- 'Reduziere Last A um 2 MW und zeige die Spannungen danach' -> change_load\n"
                "- 'Welche Leitungen hängen an Bus 5?' -> topology_query\n"
                "- 'Öffne Schalter 1' -> change_switch_state\n"
                "- 'Was ist die Nennspannung von Bus 1?' -> query_element_data\n"
                "- 'Was ist die obere Spannungsgrenze von Bus 1?' -> query_element_data\n"
                "- 'Was sind die obere und untere Spannungsgrenze von Bus 1?' -> query_element_data\n"
                "- 'Gib mir uknom von Bus 1' -> query_element_data\n"
                "- 'Welche Attribute hat Bus 1?' -> list_element_attributes\n\n"

                "Step contracts are background only. Use them only to avoid impossible step names if required_steps is not empty.\n"
                "Step contracts:\n"
                "{step_contracts}\n\n"
        ...
        self.decomposition_prompt = ChatPromptTemplate.from_messages([
            (
                ...
                "- Do not split compound attribute/data requests such as 'obere und untere Spannungsgrenze', 'Grenzwerte', 'min und max', or 'upper and lower limits'. These are one query_element_data request.\n"
                "Return only structured output.\n\n"
                "{format_instructions}"
            ),
            (
